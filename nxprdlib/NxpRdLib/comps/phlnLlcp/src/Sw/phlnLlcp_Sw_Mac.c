...
* Total Size: NNNbytes
* ***************************************************************************************************************** */
void *gphlnLlcp_MacDataParams;
uint32_t gphlnLlcp_MacType;
...
phStatus_t phlnLlcp_Sw_MacInit(phlnLlcp_Sw_DataParams_t * pDataParams, uint8_t bConfig, uint16_t *wSymmTime, uint16_t *wLtoTime)
{
    ...
    {
#ifdef NXPBUILD__PHPAL_I18092MPI
        /* Check if DID is Disabled as per LLCP 1.1 section 6.2.1. */
        ...
        /* Check if NAD is Disabled as per LLCP 1.1 section 6.2.1 */
        PH_CHECK_SUCCESS_FCT(wStatus, phlnLlcp_MacGetConfig(PHPAL_I18092MPI_CONFIG_NAD, &wValue));
        if ((uint16_t)(wValue & 0xFF00) != PH_OFF)
        {
            return (PH_ERR_PROTOCOL_ERROR | PH_COMP_LN_LLCP);
        }

        /* Check if FSL value is 11b as per LLCP 1.1 section 6.2.1. */
...
#endif /* NXPBUILD__PHPAL_I18092MPI */
#ifndef NXPBUILD__PHPAL_I18092MPI
    ...
    else
    {
        ...
        wValue = wValue & 0x000F;
        dwValue = (uint32_t)(PHLN_LLCP_RWT_MIN_US * ((uint32_t)(1 << wValue)));
        dwValue /= 1000;
        if ((pDataParams->sLocalLMParams.bLto * 10) < (dwValue * 2))
        {
            return (PH_ERR_PROTOCOL_ERROR | PH_COMP_LN_LLCP);
        }
...
#endif /* NXPBUILD__PHPAL_I18092MT */
#ifndef NXPBUILD__PHPAL_I18092MT
        return (PH_ERR_USE_CONDITION | PH_COMP_LN_LLCP);
#endif /* NXPBUILD__PHPAL_I18092MT */
    }

    /* Create LLC SYMM and LTO Timer during Init and never delete these timers. */
    PH_CHECK_SUCCESS_FCT(wStatus, phlnLlcp_Timers_InitSym(&phlnLlcp_SymTimerCallback));
    PH_CHECK_SUCCESS_FCT(wStatus, phlnLlcp_Timers_InitLto(&phlnLlcp_LtoTimerCallback));

    /* Enable the timers in LTO. */
    if (pDataParams->sRemoteLMParams.bLto == 0)
    {
    ...
    else
    {
        ...
        *wLtoTime = (uint16_t)(pDataParams->sRemoteLMParams.bLto * 10) + ((pDataParams->sRemoteLMParams.bLto * PHLN_LLCP_LTO_DELTA_PERCENTAGE)/10) ;
    }

    if (bConfig == PHLN_LLCP_TARGET)
    {
        /* Set SYMM timer value twice or thrice less then FWT. */
        *wSymmTime = (uint16_t) ((dwValue - ((dwValue * PHLN_LLCP_SYMM_DELTA_PERCENTAGE)/100)) / 2);
    }
    else
    {
        /* Enable the timers in Single shot mode for SYMM. */
        if (pDataParams->sLocalLMParams.bLto == 0)
        {
        ...
        else
        {
            ...
            *wSymmTime = (uint16_t)((pDataParams->sLocalLMParams.bLto * 10) -
                ((pDataParams->sLocalLMParams.bLto * PHLN_LLCP_SYMM_DELTA_PERCENTAGE)/10));
        }
    }
...
phStatus_t phlnLlcp_MacDeactivation()
{
...
#endif /* NXPBUILD__PHPAL_I18092MPI */
#ifdef NXPBUILD__PHPAL_I18092MT
    uint16_t   PH_MEMLOC_REM wRxBuffLen;
...
void phlnLlcp_Sw_MacHAL_ShutDown()
{
#ifdef NXPBUILD__PHPAL_I18092MPI
    phpalI18092mPI_Sw_DataParams_t PH_MEMLOC_REM * pPalI18092mPIDataParams;
#endif /* NXPBUILD__PHPAL_I18092MPI */

#ifdef NXPBUILD__PHPAL_I18092MT
    phpalI18092mT_Sw_DataParams_t  PH_MEMLOC_REM * pPalI18092mTDataParams;
#endif /* NXPBUILD__PHPAL_I18092MT */
