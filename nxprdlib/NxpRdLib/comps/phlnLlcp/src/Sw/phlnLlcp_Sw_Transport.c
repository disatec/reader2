...
* $Revision: 4258 $ (NFCRDLIB_V4.010.03.001609 : 4184 $ (NFCRDLIB_V4.010.01.001603 : 3849 ))
                                     ...
                                     )
{
    ...
    baPdu[wLength++] = (uint8_t)((wLMiu & 0xFF00) >> 8);
    baPdu[wLength++] = (uint8_t)(wLMiu & 0xFF);

    if((psSocket->bRsap == 0x01) && (bPtype == PHLN_LLCP_PTYPE_CONNECT))
    {
                                    ...
                                    uint8_t bSend)
{
    ...
    baFrmr[wLength++] = ((uint8_t)(bReason << 4) | bPtype);
    baFrmr[wLength++] = bSeq;
    baFrmr[wLength++] = (psSocket->sSeq.bSendState_Vs << 4) | psSocket->sSeq.bRxState_Vr;
    baFrmr[wLength++] = (psSocket->sSeq.bSendAck_Vsa << 4) | psSocket->sSeq.bRxAck_Vra;
