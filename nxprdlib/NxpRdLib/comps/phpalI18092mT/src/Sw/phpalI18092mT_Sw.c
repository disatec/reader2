...
* $Revision: 4258 $ (NFCRDLIB_V4.010.03.001609 : 4184 $ (NFCRDLIB_V4.010.01.001603 : 3837 ))
    ...
    )
{
    ...
    */
    ...
    {
        /* Stop timer */
        if((!pDataParams->bRtoxDisabled) && (pDataParams->dwTimerId != PH_PLATFORM_INVALID_TIMER_ID))
        {
            PH_CHECK_SUCCESS_FCT(statusTmp, phPlatform_Timer_Stop(pDataParams->pPlatform, pDataParams->dwTimerId));
            PH_CHECK_SUCCESS_FCT(statusTmp, phPlatform_Timer_Reset(pDataParams->pPlatform, pDataParams->dwTimerId));
        }
    }
#endif /* NXPBUILD__PH_PLATFORM */
    ...
    )
{
    ...
    if(pDataParams->bDid != pFrame[PHPAL_I18092MT_SW_WUP_REQ_LENGTH- 1])
    {
        return PH_ADD_COMPCODE(PH_ERR_PROTOCOL_ERROR, PH_COMP_PAL_I18092MT);
    }
    ...
    )
{
    ...
    if ((wAtrReqLength == 0x00) || (pAtrReq == NULL))
    {
    ...
    * shall disable to receive a subsequent ATR_REQ. */
    ...
    do
    {
        ...
        if (status == PH_ERR_SUCCESS)
        {
            ...
            {
                if (memcmp(&pFrame[PHPAL_I18092MT_SW_ATR_REQ_NFCID3_POS + 1], &pAtrRes[0], (PHPAL_I18092MT_NFCID3_LENGTH - 2)) != 0)
                {
    ...
    */
    ...
    memmove(pDataParams->aNfcid3i, &pFrame[PHPAL_I18092MT_SW_ATR_REQ_NFCID3_POS], PHPAL_I18092MT_NFCID3_LENGTH);  /* PRQA S 3200 */
    ...
    */
    /* copy LRi */
    pDataParams->bLri = (pFrame[PHPAL_I18092MT_SW_ATR_REQ_PPI_POS] >> PHPAL_I18092MT_SW_ATR_PPT_LRT_BIT) & 0x03;
    ...
    */
    /* Copy the NFC ID3 */
    memmove(pDataParams->aNfcid3t, &pAtrRes[0], PHPAL_I18092MT_NFCID3_LENGTH); /* PRQA S 3200 */
    memmove(&aCmd[bCmdLen], &pAtrRes[0], PHPAL_I18092MT_NFCID3_LENGTH);  /* PRQA S 3200 */
     ...
     */
    pDataParams->bLrt = (pAtrRes[bCmdLen - 1]  >> PHPAL_I18092MT_SW_ATR_PPT_LRT_BIT) & 0x03;
    aCmd[bCmdLen] = pAtrRes[bCmdLen - 1] & 0x30;

    if (pDataParams->bNadEnabled)
    {
        aCmd[bCmdLen] |= PHPAL_I18092MT_SW_ATR_REQ_PPI_NAD_USED;
    }
    if (pAtrRes[bCmdLen - 1] & PHPAL_I18092MT_SW_ATR_RES_PPT_GT_AVAILABLE)
    {
    ...
    )
{
    ...
    do
    {
        ...
        if(bDataRec == PH_ON)
        {
            ...
            case PHPAL_I18092MT_SW_CMD_RLS_REQ:
                ...
                {
                    bDataRec = PH_ON;
                    status = PH_ADD_COMPCODE(PH_ERR_SUCCESS_RELEASED, PH_COMP_PAL_I18092MT);
                }
                break;
            ...
            case PHPAL_I18092MT_SW_CMD_DEP_REQ:
                ...
                if (status == PH_ERR_SUCCESS)
                {
                    ...
                    if(bPfb & PHPAL_I18092MT_SW_PFB_PDU_TYPE_MASK)
                    {
                    ...
                    if (wRxOverlappedStartPos)
                    {
                        /* Restore overlapped bytes */
                        memcpy(&pRxBuf[wRxOverlappedStartPos - 1], &aRxOverlapped[0], (uint32_t)((wRxLen - wDepReqLen) + 1));  /* PRQA S 3200 */
                        ...
                        if ((pDataParams->bCmdtype == PHPAL_I18092MT_SW_CMD_ATN) && (bPrevPni != pDataParams->bPni))
                        {
                            ...
                            memcpy(&pRxBuf[wRxLen], pPayload, wPayloadLength);    /* PRQA S 3200 */
                        }
                    }
                    ...
                    /* Reset RxBuffer Start Position :this is added to take care of in case of last packet of chaining */
                    PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_SetConfig(pDataParams->pHalDataParams, PHHAL_HW_CONFIG_RXBUFFER_STARTPOS, 0));
    ...
    *pTransportDataBytesLength = (uint16_t)(wRxLen - bProtLength);
    ...
    {
        if(pDataParams->dwTimerId == PH_PLATFORM_INVALID_TIMER_ID)
        {
            PH_CHECK_SUCCESS_FCT(statusTmp, phPlatform_Timer_Create(pDataParams->pPlatform,  &pDataParams->dwTimerId));
        }

        if(pDataParams->dwTimerId != PH_PLATFORM_INVALID_TIMER_ID)
        {
            PH_CHECK_SUCCESS_FCT(statusTmp, phPlatform_Timer_Reset(pDataParams->pPlatform,  pDataParams->dwTimerId));
            PH_CHECK_SUCCESS_FCT(statusTmp, phPlatform_Timer_Start(
    ...
    )
{
    ...
    do
    {
        ...
        if (wOption & PH_TRANSMIT_LEAVE_BUFFER_BIT)
        {
            PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_GetConfig(
                pDataParams->pHalDataParams,
        ...
        else
        {
            ...
            if ((wBytesToSend == wTransportDataBytesLength) && !(wOption & PH_TRANSMIT_BUFFERED_BIT))
            {
            ...
            (pDataParams->bStateNow & PHPAL_I18092MT_SW_STATE_CHAINING_LAST_BIT))
        {
            ...
            * Header should be only be updated once again from next Chained packet. */
            if (!(wRxOption & PH_TRANSMIT_LEAVE_BUFFER_BIT))
            {
        ...
        if ((!bForceSend) && (wOption & PH_TRANSMIT_BUFFERED_BIT))
        {
        ...
        if (pDataParams->bStateNow == (PHPAL_I18092MT_SW_STATE_TX | PHPAL_I18092MT_SW_STATE_CHAINING_BIT))
        {
            ...
            else
            {
                ...
                if (wTransportDataBytesLength > 0)
                {
                                      ...
                                      )
{
    ...
    case PHPAL_I18092MT_CONFIG_NAD:
        if (wValue & PHPAL_I18092MT_MASK_NAD_ENABLE)
        {
                                      ...
                                      )
{
    ...
    case PHPAL_I18092MT_CONFIG_NAD:
        if (pDataParams->bNadEnabled)
        {
            *pValue = (uint16_t)(0x0100 | pDataParams->bNadVal);
        }
        else
        {
            *pValue = 0;
