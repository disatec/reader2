 ...
 * Project:  PN7462AU
 *
 * $Date: 2016-07-05 00:18:34 +0530 (Tue, 05 Jul 2016) $
 * $Author: Purnank G (ing05193) $
 * $Revision: 5431 $ (r4.030.00.011627)
 *
 */
...
/*   DEFINITIONS                                                                                                     */
/*********************************************************************************************************************/
#define PHPAL_MIFARE_KEYA           0x0A
#define PHPAL_MIFARE_KEYB           0x0B
#define PHHAL_HW_TXSTARTSYMBOL0     0x01
#define PHHAL_HW_TXSTARTSYMBOL1     0x02
#define PHHAL_HW_15693_HIGH         0x00
#define PHHAL_HW_15693_FASTHIGH     0x01
#define PHHAL_HW_EPC_423_MAN2       0x00
#define PHHAL_HW_EPC_423_MAN4       0x01
#define PHHAL_HW_EPC_847_MAN2       0x02
#define PHHAL_HW_EPC_847_MAN4       0x03
#define PHHAL_HW_SINGLESIZEUID      0x04
#define PHHAL_HW_CRC5_PRESET        0x04
    ...
    uint16_t wRdlibRxBufSize)
{
    ...
    if (wRdlibTxBufSize == 0 || wRdlibRxBufSize == 0)
    {
    ...
    uint16_t wConfig, uint16_t wValue)
{
    ...
    case PHHAL_HW_CONFIG_TXDATARATE_FRAMING:
    {
        if (((wValue & PHHAL_HW_RF_FRAMING_OPTION_MASK) == PHHAL_HW_RF_TYPE_ACTIVE_FRAMING)
                && ((wValue & PHHAL_HW_RF_DATARATE_OPTION_MASK) < 4 ))
        {
            if(pDataParams->wTargetMode)
            {
                    ...
                    && ((wValue & PHHAL_HW_RF_DATARATE_OPTION_MASK) < 4 ))
        {
            if(pDataParams->wTargetMode)
            {
                status = phhalRf_LoadProtocol_Target((phhalRf_LP_T_Tx_t)(PHHAL_HW_CARDTYPE_ISO14443A +
                            (wValue & PHHAL_HW_RF_DATARATE_OPTION_MASK) + 1) , E_PHHAL_RF_LP_T_RX_KEEP_ASIS);
            }
            else
            {

                status = phhalRf_LoadProtocol_Initiator((phhalRf_LP_I_Tx_t)(PHHAL_HW_CARDTYPE_ISO14443A +
                            (wValue & PHHAL_HW_RF_DATARATE_OPTION_MASK) + 1) ,E_PHHAL_RF_LP_I_RX_KEEP_ASIS);
                    ...
                    && ((wValue & PHHAL_HW_RF_DATARATE_OPTION_MASK) < 4 ))
        {
            if(pDataParams->wTargetMode)
            {
                status = phhalRf_LoadProtocol_Target((phhalRf_LP_T_Tx_t)(PHHAL_HW_CARDTYPE_ISO14443B +
                            (wValue & PHHAL_HW_RF_DATARATE_OPTION_MASK) + 8) ,E_PHHAL_RF_LP_T_RX_KEEP_ASIS);
            }
            else
            {
                status = phhalRf_LoadProtocol_Initiator((phhalRf_LP_I_Tx_t)(PHHAL_HW_CARDTYPE_ISO14443B +
                            (wValue & PHHAL_HW_RF_DATARATE_OPTION_MASK) + 5) ,E_PHHAL_RF_LP_I_RX_KEEP_ASIS);
            }
        }
        ...
        else
        {
            ...
            {
                if((wValue & PHHAL_HW_RF_DATARATE_OPTION_MASK) > 3)
                {
                    status = PH_ERR_INVALID_PARAMETER;
                }
                else
    ...
    case PHHAL_HW_CONFIG_RXDATARATE_FRAMING:
    {
                    ...
                    && ((wValue & PHHAL_HW_RF_DATARATE_OPTION_MASK) < 4 ))
        {
            if(pDataParams->wTargetMode)
            {
                ...
                (phhalRf_LP_T_Rx_t)(PHHAL_HW_CARDTYPE_ISO14443A + (wValue & PHHAL_HW_RF_DATARATE_OPTION_MASK) + 1) );
            }
            else
            {
                status = phhalRf_LoadProtocol_Initiator(E_PHHAL_RF_LP_I_TX_KEEP_ASIS,
                (phhalRf_LP_I_Rx_t)(PHHAL_HW_CARDTYPE_ISO14443A + (wValue & PHHAL_HW_RF_DATARATE_OPTION_MASK) + 1));
            }
        }
        else if (((wValue & PHHAL_HW_RF_FRAMING_OPTION_MASK) == PHHAL_HW_RF_TYPE_B_FRAMING)
                    && ((wValue & PHHAL_HW_RF_DATARATE_OPTION_MASK) < 4 ))
        {
            ...
            {
                status = phhalRf_LoadProtocol_Target(E_PHHAL_RF_LP_T_TX_KEEP_ASIS ,
                (phhalRf_LP_T_Rx_t)(PHHAL_HW_CARDTYPE_ISO14443B + (wValue & PHHAL_HW_RF_DATARATE_OPTION_MASK) + 8));
            }
            else
            {
                status = phhalRf_LoadProtocol_Initiator((phhalRf_LP_I_Tx_t)E_PHHAL_RF_LP_I_TX_KEEP_ASIS ,
                (phhalRf_LP_I_Rx_t)(PHHAL_HW_CARDTYPE_ISO14443B + (wValue & PHHAL_HW_RF_DATARATE_OPTION_MASK) + 8));
            }
        }
        else if (((wValue & PHHAL_HW_RF_FRAMING_OPTION_MASK) == PHHAL_HW_RF_TYPE_F_FRAMING)
                    && ((wValue & PHHAL_HW_RF_DATARATE_OPTION_MASK) < 4 ))
        ...
        else
        {
            ...
            case PHHAL_HW_CARDTYPE_ISO14443A:
            {
                if ((wValue & PHHAL_HW_RF_DATARATE_OPTION_MASK) < 4 )
                {
                    status = phhalRf_LoadProtocol_Initiator(E_PHHAL_RF_LP_I_TX_KEEP_ASIS,
                    (phhalRf_LP_I_Rx_t)(PHHAL_HW_CARDTYPE_ISO14443A + (wValue & PHHAL_HW_RF_DATARATE_OPTION_MASK) + 1));
                }
            ...
            case PHHAL_HW_CARDTYPE_ISO14443B:
            {
                ...
                {
                    status = phhalRf_LoadProtocol_Initiator(E_PHHAL_RF_LP_I_TX_KEEP_ASIS,
                    (phhalRf_LP_I_Rx_t)(PHHAL_HW_CARDTYPE_ISO14443B + (wValue & PHHAL_HW_RF_DATARATE_OPTION_MASK) + 5));
                }
                else
                {
                    status = PH_ERR_INVALID_PARAMETER;
    ...
    {
        /* fdt in millisecond */
        pDataParams->dwFdt = (((uint32_t)(wValue)) * 1000);
        status = phhalRf_PCD_SetExchgFdt((uint32_t)pDataParams->dwFdt);
    }
    break;
    ...
    case PHHAL_HW_CONFIG_CRCTYPE:
    {
        ...
        status = phhalRf_SetConfig(PHHAL_RF_CONFIG_CRCINVERTED, (uint32_t)((wValue & PH_COMP_MASK)>>8));
    ...
    case PHHAL_HW_CONFIG_TXWAIT_MS:
    {
        ...
        dwValueTmp = (uint32_t)(wValue*1000);
...
phStatus_t phhalHw_PN7462AU_Wait(phhalHw_PN7462AU_DataParams_t *pDataParams PH_UNUSED, uint8_t bUnit, uint16_t wTimeout)
{
    ...
    if(PHHAL_HW_TIME_MILLISECONDS == bUnit)
    {
        ...
        dwRdLibTimeout = (1000 * (uint32_t)wTimeout);
...
)
{
    ...
    else if(!pDataParams->dwI15693Sof)
    {
    ...
    else if (status == PH_ERR_COLLISION_ERROR)
    {
        ...
        *pRxLength = gphhalRf_RxParams.wRxBytes;
        ...
        gphhalRf_RxParams.bRxBits = (uint8_t)(gphhalRf_RxParams.bRxCollPos + 8 - (*pRxLength * 8));
        if(pDataParams->wRxBufStartPos)
        {
            *pRxLength = *pRxLength + (pDataParams->wRxBufStartPos);
        }
        if (gphhalRf_RxParams.bRxBits == 0)
        {
...
#endif
        if ((wOption & PH_TRANSMIT_PREV_FRAME) && (wOption != PH_TRANSMIT_PREV_FRAME))
        {
            return PH_ADD_COMPCODE(PH_ERR_INVALID_PARAMETER, PH_COMP_HAL);
        }
                                        ...
                                        uint16_t * pRxLength, uint16_t * pProtParams)
{
    ...
    if (status == PH_ERR_SUCCESS)
    {
        ...
        *(((uint8_t *)(pProtParams)) + 0) = sProtParams.eBaudRateDetected;
        *(((uint8_t *)(pProtParams)) + 1) = sProtParams.eProtocolDetected;

        /** SC3625 **/

        *pProtParams = *pProtParams & 0x00FF;
        ...
        {
            *ppRxBuffer = (uint8_t *)(*ppRxBuffer + 1);
