                                 ...
                                 )
{
    ...
    do
    {
        ...
        if ((status & PH_ERR_MASK) == PH_ERR_SUCCESS)
        {
            ...
            *pAtrResLength = (uint8_t)wFrameLength;
            ...
            pDataParams->bLrt = (pAtrRes[PHPAL_I18092MPI_SW_ATR_RES_PPT_POS] >> 4) & 0x3;
        ...
        else
        {
            ...
            if (((status & PH_ERR_MASK) == PH_ERR_INTEGRITY_ERROR) && (bRetryCount == 0))
            {
    ...
    if ((wPreviousStatus) && (status != PH_ERR_SUCCESS))
    {
                                      ...
                                      )
{
    ...
    do
    {
        ...
        if ((!(pDataParams->bStateNow & PHPAL_I18092MPI_SW_STATE_CHAINING_BIT)) && (!(wOption & PH_TRANSMIT_BUFFERED_BIT)) && (wBytesToSend == wTxLength))
        {
                                     ...
                                     )
{
    ...
    if (wOption &  (uint16_t)~(uint16_t)
        (
        PH_RECEIVE_CHAINING | PH_RECEIVE_CHAINING_BUFSIZE
        ))
    {
        return PH_ADD_COMPCODE(PH_ERR_INVALID_PARAMETER, PH_COMP_PAL_I18092MPI);
    }
...
phStatus_t phpalI18092mPI_Sw_Wakeup(phpalI18092mPI_Sw_DataParams_t * pDataParams)
{
    ...
    /* Build up frame data. */
    memcpy(aCmd, pDataParams->aNfcid3i, PHPAL_I18092MPI_NFCID3_LENGTH); /* PRQA S 3200 */
    bCmdLen = PHPAL_I18092MPI_NFCID3_LENGTH;
    aCmd[bCmdLen++] = pDataParams->bDid;
