...
* $Revision: 4258 $ (NFCRDLIB_V4.010.03.001609 : 4184 $ (NFCRDLIB_V4.010.01.001603 : 3021 ))
                                    ...
                                    )
{
    ...
    {
        status = PH_ADD_COMPCODE(PH_ERR_INVALID_DATA_PARAMS, PH_COMP_PAL_I18000P3M3);
                               ...
                               )
{
    phStatus_t  PH_MEMLOC_REM status;
#ifdef NXPBUILD__PH_LOG
    uint16_t    PH_MEMLOC_REM wBufferByteLength;
#endif
    ...
    if ((status & PH_ERR_MASK) == PH_ERR_SUCCESS)
    {
        wBufferByteLength = *pRxBitLength >> 3;
        /* check for incomplete bytes */
        if (*pRxBitLength & 0x07)
        {
            ++wBufferByteLength;
        }
        ...
        )
{
    ...
    PH_ASSERT_NULL (pDataParams);
    if (bSelCmdLen != 0) PH_ASSERT_NULL (pSelCmd);
